<?php
/**
 * FIXED Question Scraper - Separates English and Tamil
 * Enhanced version that splits bilingual content
 */
...
try {
    ...
    for ($pageNum = 1; $pageNum <= 10; $pageNum++) {
        ...
        // Extract questions from this page WITH LANGUAGE SEPARATION
        $pageQuestions = extractQuestionsBilingual($html);
...
/**
 * 🔥 NEW FUNCTION: Extract questions WITH language separation
 */
function extractQuestionsBilingual($html) {
    $questions = [];
    ...
    for ($i = 0; $i < count($parts) - 1; $i += 2) {
        ...
        if ($questionText && 
            ...
            // 🔥 NEW: SEPARATE English and Tamil!
            $questionSplit = splitBilingual($questionText);
            $optionASplit = splitBilingual($options['A']);
            $optionBSplit = splitBilingual($options['B']);
            $optionCSplit = splitBilingual($options['C']);
            $optionDSplit = splitBilingual($options['D']);
            
            $questions[] = [
                // Question
                'question_en' => $questionSplit['english'],
                'question_ta' => $questionSplit['tamil'],
                // Options
                'option_a_en' => $optionASplit['english'],
                'option_a_ta' => $optionASplit['tamil'],
                'option_b_en' => $optionBSplit['english'],
                'option_b_ta' => $optionBSplit['tamil'],
                'option_c_en' => $optionCSplit['english'],
                'option_c_ta' => $optionCSplit['tamil'],
                'option_d_en' => $optionDSplit['english'],
                'option_d_ta' => $optionDSplit['tamil'],
                // Answer
...
/**
 * 🔥 NEW: Strip HTML tags and decode entities
 */
...
/**
 * 🔥 NEW: Split bilingual text into English and Tamil parts
 */
function splitBilingual($text) {
    // FIRST: Clean the text (remove HTML tags)
    $text = cleanText($text);
    ...
    $hasTamil = preg_match('/[\x{0B80}-\x{0BFF}]/u', $text);
    ...
    if (preg_match('/(.*?)([\x{0B80}-\x{0BFF}].*)$/u', $text, $matches)) {
        $englishPart = trim($matches[1]);
        $tamilPart = trim($matches[2]);
        
        // If English part is too short (< 5 chars), it might be a false split
        if (strlen($englishPart) < 5) {
            // All Tamil, no English
            return [
                'english' => null,
                'tamil' => $text
            ];
