...
if ($method === 'GET') {
    // Get all test categories (including inactive for admin)
    $exam_id = isset($_GET['exam_id']) ? $_GET['exam_id'] : null;
    $testCategory = new TestCategory($db);
    $stmt = $testCategory->getAll($exam_id, null, true); // includeInactive = true for admin
    $categories = [];
    ...
    if (!empty($data->name) && !empty($data->exam_category_id)) {
        ...
        $testCategory->image = $data->image ?? '';
        $testCategory->image_path = $data->image_path ?? '';
