 ...
 *
 * ═══════════════════════════════════════════════════════════════════
 * ANDROID OTP AUTO-FILL (SMS RETRIEVER) - IMPORTANT
 * ═══════════════════════════════════════════════════════════════════
 *
 * The Flutter app supports Android OTP auto-fill using `sms_autofill`
 * (Android SMS Retriever API). This requires a special SMS template format.
 *
 * Your approved AuthKey.io template (SID) MUST match this format:
 *
 *   <#> Your OTP is {OTP}
 *   {APP_HASH}
 *
 * Where:
 * - `{OTP}` is the 6-digit OTP value
 * - `{APP_HASH}` is the 11-character app signature hash from the Flutter app:
 *     SmsAutoFill().getAppSignature
 *
 * Notes:
 * - The `<#>` at the start is required.
 * - The hash MUST be on its own line at the END of the message.
 * - Keep the SMS short (single SMS). Long disclaimer text can break auto-fill.
 * - Debug and Release builds have DIFFERENT hashes. Use the Release hash in production.
 *
 * If you cannot change the SMS template, the app can still support keyboard OTP
 * suggestions via `AutofillHints.oneTimeCode`, but Android SMS Retriever requires the hash.
 */
