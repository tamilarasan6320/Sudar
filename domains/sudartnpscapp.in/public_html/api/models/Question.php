...
class Question {
    ...
    public function create() {
        // Validate: Must have question text in at least one language
        // AND at least one option in at least one language
        $hasEnglishQuestion = !empty($this->question_en);
        $hasTamilQuestion = !empty($this->question_ta);
        
        $hasEnglishOptions = !empty($this->option_a_en) || 
                            !empty($this->option_b_en) || 
                            !empty($this->option_c_en) || 
                            !empty($this->option_d_en);
        
        $hasTamilOptions = !empty($this->option_a_ta) || 
                          !empty($this->option_b_ta) || 
                          !empty($this->option_c_ta) || 
                          !empty($this->option_d_ta);
        
        // Valid if: has any question text AND has any options
        $hasQuestion = $hasEnglishQuestion || $hasTamilQuestion;
        $hasOptions = $hasEnglishOptions || $hasTamilOptions;
        
        if (!$hasQuestion || !$hasOptions) {
            return false; // Invalid question - no question text or no options
        }
        ...
        // Add table_data if column exists (check dynamically)
        $checkTableData = $this->conn->query("SHOW COLUMNS FROM " . $this->table_name . " LIKE 'table_data'");
        if ($checkTableData && $checkTableData->rowCount() > 0) {
        ...
        if ($checkTableData && $checkTableData->rowCount() > 0) {
            $stmt->bindParam(':table_data', $this->table_data);
        }
    ...
    public function checkDuplicate() {
        ...
        // Return the duplicate ID if found, otherwise return false
        if ($stmt->rowCount() > 0) {
