...
class OtpService {
    ...
    /**
     * Ensure OTP table exists with all required columns
     */
    ...
    /**
     * Send OTP to Mobile Number via AuthKey.io SMS
     * 
    ...
    public function sendOtp($mobile) {
        ...
        try {
            ...
            // Send SMS via AuthKey.io (PRODUCTION)
            if (!OTPConfig::isDevelopmentMode()) {
                $smsResult = $this->sendSmsViaAuthKey($mobile, $otp);
                
                if (!$smsResult['success']) {
                    error_log("AuthKey.io SMS Error for $mobile: " . json_encode($smsResult));
    ...
    /**
     * Resend OTP with rate limit checks
     */
     ...
     * 
     * API: https://api.authkey.io/request?authkey=dc0b07c812ca4934&mobile=MOBILE&country_code=91&sid=25451&otp=OTP
     */
    private function sendSmsViaAuthKey($mobile, $otp) {
        $url = OTPConfig::getApiUrl($mobile, $otp);
        
        // Log the API call
        error_log("AuthKey.io API Call: $url");
        ...
        // Log response
        error_log("AuthKey.io Response: HTTP $httpCode - $response");
        
        if ($error) {
            error_log("AuthKey.io cURL Error: $error");
            return [
                'success' => false,
    ...
    /**
     * Get Resend Count for Mobile (today)
     */
